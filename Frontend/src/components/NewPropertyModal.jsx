...
async function createUnitsForProperty(propertyId, units) {
  ...
  try {
    ...
    console.log(`${units.length} units created successfully for property ${propertyId}`);
  } catch (error) {
...
const TextArea = ({ id, name, value, onChange, placeholder, required, rows = 3, className = "", ...props }) => (
  <textarea
    id={id || name}
    name={name}
    value={value}
...
const Select = ({ id, name, value, onChange, required, children, className = "", ...props }) => (
...
const NewPropertyModal = ({ isOpen, onClose, onSubmit, isLoading }) => {
  ...
  const handleSuggestionClick = (suggestion) => {
    setFormData(prev => ({
      ...prev,
      address: suggestion.address.streetNameAndNumber || suggestion.address.freeformAddress || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.className = 'fixed bottom-4 right-4 bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded shadow-lg';
      toast.innerHTML = `
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <FormSection title="Property Information">
                      ...
                      >
                        {suggestions.length > 0 ? (
                          suggestions.map((suggestion, index) => (
                            ...
                            >
                              <div className="font-medium text-gray-800">{suggestion.address.freeformAddress}</div>
                              <div className="text-sm text-gray-500">{suggestion.address.countrySubdivisionName}, {suggestion.address.country}</div>
                            </li>
                          ))
                        ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                ...
                <div>
                  <Label htmlFor="postal_code" required>Postal Code</Label>
                  <Input
                    id="postal_code"
                    name="postal_code"
            ...
            <AnimatePresence>
              {isApartmentComplex && (
                ...
                >
                  <FormSection title="Apartment Details" className="pt-4 border-t border-gray-200">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <Label htmlFor="num_floors" required>Number of Floors</Label>
                        <Input
                          id="num_floors"
                          name="num_floors"
                          type="number"
                      ...
                      <div>
                        <Label htmlFor="units_per_floor" required>Units Per Floor</Label>
                        <Input
                          id="units_per_floor"
                      ...
                      <AnimatePresence>
                        {!formData.auto_generate_units && (
                          <motion.div
                            ...
                            className="mt-4"
                          >
                            <Label htmlFor="manual_units" required>Enter Unit Numbers</Label>
                            <TextArea
                              id="manual_units"
                              name="manual_units"
                              ...
                              required={!formData.auto_generate_units && isApartmentComplex}
                            />
