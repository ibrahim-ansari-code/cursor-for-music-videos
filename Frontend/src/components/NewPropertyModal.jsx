...
const NewPropertyModal = ({
  ...
  // Reset highlightedIndex when suggestions or dropdown visibility changes
  useEffect(() => {
  ...
  const debouncedFetch = useCallback(
    debounce(async (query) => {
      ...
      try {
        ...
        if (latestQueryRef.current === query) {
          setSuggestions([]);
          setHighlightedIndex(-1); // Reset highlight on error
        }
  ...
  const handleChange = (e) => {
    ...
    if (name === "address") {
      ...
      if (value.length >= 3) {
        ...
        setSuggestions([]);
        setShowSuggestions(false);
        setHighlightedIndex(-1); // Reset highlight when hiding
