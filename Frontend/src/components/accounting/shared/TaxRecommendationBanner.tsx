    ...
    if (isLoadingSmartTax) {
      return (
        <motion.div
          ...
          className="absolute top-12 left-0 right-0 z-10 bg-white border border-emerald-200 rounded-lg shadow-lg p-3"
        >
      ...
      <AnimatePresence>
        {smartTaxRecommendation && smartTaxRecommendation.tax_name && !isDuplicateRecommendation && (
          <motion.div
            ...
            className="absolute bottom-full left-0 mb-1 z-50 bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 rounded-lg shadow-lg pointer-events-auto"
            style={{ 
          ...
          >
            <div className="flex items-center justify-between px-3 py-2.5">
              ...
              <button
                ...
                className="flex-shrink-0 bg-emerald-500 hover:bg-emerald-600 text-white px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ml-3"
              >
                Apply
              </button>
            </div>
    ...
    <AnimatePresence>
      {smartTaxRecommendation && smartTaxRecommendation.tax_name && !isDuplicateRecommendation && (
        <motion.div
          ...
          animate={{ opacity: 1, height: 'auto' }}
          exit={{ opacity: 0, height: 0 }}
          className={`bg-green-50 border border-green-200 rounded-lg p-4 mb-4 ${className}`}
        >
