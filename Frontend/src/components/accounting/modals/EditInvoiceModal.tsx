...
import type { TaxDetail, Invoice } from '../../../types/accounting';
...
/**
 * Refactored EditInvoiceModal with extracted business logic and modular components
 * - Uses the same hooks and components as NewInvoiceModal for consistency
 * - Initializes form data from existing invoice
...
const EditInvoiceModal: React.FC<EditInvoiceModalProps> = ({
  ...
  const convertInvoiceToFormData = (invoice: Invoice) => {
    return {
  ...
  const {
    ...
    initialData: invoiceData ? convertInvoiceToFormData(invoiceData) : undefined,
    mode: 'edit',
    onSuccess: () => {
      onSuccess?.();
      onClose();
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Populate form data from invoice when available
      if (invoiceData) {
        setFormData(convertInvoiceToFormData(invoiceData));
      }
    } else {
      // Cleanup when modal closes
      clearRecommendations();
    }
  }, [isOpen, invoiceData, setFormData, clearRecommendations]);
