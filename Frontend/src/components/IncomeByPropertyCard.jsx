  ...
  return (
    <div className="flex flex-col h-full p-4 bg-white rounded-lg shadow-sm">
      <div className="flex items-center mb-4">
        <h3 className="text-base font-medium text-gray-800">Income by Property</h3>
      </div>
            ...
            <>
              ...
              <div className="ml-14 mr-10 mt-4 h-52 flex items-end justify-between">
                {properties.map((property, index) => (
                  <div key={property.id || index} className="flex flex-col items-center relative" style={{ width: `${100 / properties.length}%`, maxWidth: '120px' }}>
                    <div 
                      className="w-16 bg-blue-500 rounded-t-sm"
                      style={{ 
                        height: `${Math.min((property.monthlyIncome / 6000) * 100, 100)}%`,
                        maxHeight: '100%'
                      }}
                    ></div>
              ...
              {/* Occupancy Line */}
              <svg className="absolute inset-0 ml-14 mr-10 mt-4 h-52 pointer-events-none" preserveAspectRatio="none">
                <path 
