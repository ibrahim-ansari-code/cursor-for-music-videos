      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 space-y-4">
            ...
            <motion.div className="bg-white rounded-xl border border-gray-200 p-4">
              <h4 className="font-semibold text-gray-900 mb-3">Industrial Details</h4>
              <div className="grid grid-cols-4 gap-2">
                {totalSquareFeet && (
                  <div className="text-center p-2 bg-orange-50 rounded-lg">
                    <Square className="h-4 w-4 mx-auto text-orange-600 mb-1" />
                    <div className="text-sm font-semibold text-gray-900">{Number(totalSquareFeet).toLocaleString()}</div>
                    <div className="text-xs text-gray-600">Total SF</div>
                  </div>
                )}
                {clearHeight && (
                  <div className="text-center p-2 bg-blue-50 rounded-lg">
                    <Gauge className="h-4 w-4 mx-auto text-blue-600 mb-1" />
                    <div className="text-sm font-semibold text-gray-900">{clearHeight}'</div>
                    <div className="text-xs text-gray-600">Clear Height</div>
                  </div>
                )}
                {loadingDocks != null && (
                  <div className="text-center p-2 bg-green-50 rounded-lg">
                    <Package className="h-4 w-4 mx-auto text-green-600 mb-1" />
                    <div className="text-sm font-semibold text-gray-900">{loadingDocks}</div>
                    <div className="text-xs text-gray-600">Loading Docks</div>
                  </div>
                )}
                {railAccess && (
                  <div className="text-center p-2 bg-purple-50 rounded-lg">
                    <Train className="h-4 w-4 mx-auto text-purple-600 mb-1" />
                    <div className="text-xs font-semibold text-gray-900">Yes</div>
                    <div className="text-xs text-gray-600">Rail Access</div>
                  </div>
                )}
              </div>
            </motion.div>
