...
const RentTrackerRow = memo(({ index, style, data }) => {
  ...
  return (
    <div 
      ...
      className={`flex items-stretch w-full ${bgColor} hover:bg-blue-50 transition-colors border-b border-gray-200`}
    >
      {/* Tenant */}
      ...
      <div className="px-4 py-4 overflow-hidden" style={{ width: `${columnWidthPercentages.property}%`, boxSizing: 'border-box' }}>
        <div className="text-sm text-gray-900 text-left truncate" title={sanitizePropertyName(rent?.property_name || '')}>
          {sanitizePropertyName(rent?.property_name || '')}
        </div>
        {rent?.unit_name && (
          <div className="text-xs text-gray-500 text-left truncate" title={sanitizePropertyName(rent?.unit_name || '')}>
      ...
      <div className={`px-4 py-4 ${isSmallScreen ? '' : 'flex items-center justify-center'}`} style={{ width: `${columnWidthPercentages.balanceDue}%`, boxSizing: 'border-box' }}>
        <div className={`${isSmallScreen ? 'space-y-2' : ''}`}>
          <div className={`text-sm font-medium text-center ${
            Number.parseFloat(rent?.remaining_due ?? 0) > 0 ? "text-red-600" : "text-gray-500"
          }`}>
          ...
          {isSmallScreen && (
            <div className="flex flex-col space-y-1">
              ...
              <button
                ...
                className="text-blue-600 hover:text-blue-900 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 rounded px-2 py-1 border border-blue-300 hover:border-blue-500"
                tabIndex={0}
              >
                Record Payment
              </button>
      ...
      {!isSmallScreen && columnWidthPercentages.actions > 0 && (
        <div className="px-4 py-4 flex items-center justify-center" style={{ width: `${columnWidthPercentages.actions}%`, boxSizing: 'border-box' }}>
          <button
            onClick={() => handleRecordPayment(rent)}
            onKeyDown={handleKeyDown}
            className="text-blue-600 hover:text-blue-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 rounded px-2 py-1"
            tabIndex={0}
          >
            Record Payment
          </button>
...
const RentTrackerVirtualTable = ({ 
  ...
  return (
    <div className="bg-white shadow rounded-lg overflow-hidden" ref={containerRef}>
      {/* Fixed Header */}
      <div className="border-b-2 border-gray-200 bg-gray-50">
        <div className="flex w-full" style={{boxSizing: 'border-box'}}>
          ...
          <div className="px-4 py-3" style={{ width: `${columnWidthPercentages.monthlyRent}%`, boxSizing: 'border-box' }}>
            <div className="text-center text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">
              Rent
            </div>
          </div>
