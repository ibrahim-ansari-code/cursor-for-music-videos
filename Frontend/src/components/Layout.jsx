                    ...
                    {user?.profile_image_url ? (
                        <img 
                            ...
                            onError={(e) => { 
                                ...
                                console.warn('Header avatar failed to load:', user.profile_image_url);
                                e.target.style.display = 'none';
                             }}
                         />
                    ) : (
                        <span className="text-xs font-medium">{getInitials(user?.first_name, user?.last_name)}</span>
                    )}
                </div>
                <div className="hidden md:block">
                  <p className="text-sm font-medium text-gray-900 group-hover:text-blue-600 transition-colors">{user?.first_name} {user?.last_name}</p>
