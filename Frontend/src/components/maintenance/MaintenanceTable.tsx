...
import type { MaintenanceRequest, MaintenanceTableProps, Tenant } from '../../types/tenant';
...
// Helper function to format tenant name - handles both API response and Tenant type
const getTenantName = (tenant?: Tenant | { first_name?: string; last_name?: string; company_name?: string; tenant_type?: string } | null): string => {
  ...
  if (!requests || requests.length === 0) {
    return (
      <div className="dark-panel dark-shadow rounded-lg p-8 text-center dark-divider border">
        ...
        <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No maintenance requests found</h3>
        <p className="text-gray-500 dark:text-gray-400">No maintenance requests match the current filter.</p>
      </div>
    );
  }
            ...
            return (
              <tr key={request.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{request.issue_title}</div>
                  <div className="text-sm text-gray-500 dark:text-gray-400 truncate" style={{ maxWidth: '250px' }}>
                    {request.description}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 transition-colors duration-300">
