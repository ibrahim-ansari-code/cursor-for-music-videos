...
@pytest.fixture
def run_manager(mock_agents_client, mock_tool_manager):
    """Create RunManager instance with mocks"""
    return RunManager(mock_agents_client, "assistant_123", mock_tool_manager)


class TestRunManager:
    """Test cases for RunManager class"""

    async def test_add_message_and_run_success(self, run_manager, mock_agents_client):
        ...
        mock_agents_client.runs.create.return_value = mock_run
        ...
        mock_agents_client.runs.create.assert_called_once_with(
            thread_id=thread_id,
            agent_id="assistant_123"
        )

    async def test_add_message_and_run_failure(self, run_manager, mock_agents_client):
        """Test failure during message addition and run creation"""
        # Arrange
        thread_id = "thread_123"
        message_content = "Hello!"
        mock_agents_client.runs.create.side_effect = Exception("Run creation failed")

        # Act & Assert
        with pytest.raises(Exception, match="Run creation failed"):
            await run_manager.add_message_and_run(thread_id, message_content)

    async def test_get_run_status_completed(self, run_manager, mock_agents_client):
        ...
        mock_agents_client.runs.get.return_value = mock_run
        ...
        mock_agents_client.runs.get.assert_called_once_with(
            thread_id=thread_id,
            run_id=run_id
        )

    async def test_get_run_status_requires_action(self, run_manager, mock_agents_client, mock_tool_manager):
    ...
    async def test_get_run_status_requires_action_no_session(self, run_manager, mock_agents_client):
    ...
    async def test_get_run_status_failed(self, run_manager, mock_agents_client):
    ...
    async def test_get_run_status_failed_no_error_details(self, run_manager, mock_agents_client):
    ...
    async def test_get_run_status_in_progress(self, run_manager, mock_agents_client):
    ...
    async def test_get_run_status_api_failure(self, run_manager, mock_agents_client):
        """Test failure when getting run status"""
        # Arrange
        thread_id = "thread_123"
        run_id = "run_123"
        mock_agents_client.runs.get.side_effect = Exception("API error")

        # Act & Assert
        with pytest.raises(Exception, match="API error"):
            await run_manager.get_run_status(thread_id, run_id)

    async def test_tool_execution_integration(self, run_manager, mock_agents_client, mock_tool_manager):
    ...
    async def test_various_run_statuses(self, run_manager, mock_agents_client):
    ...
    async def test_message_creation_failure(self, run_manager, mock_agents_client):
        """Test failure during message creation"""
        # Arrange
        thread_id = "thread_123"
        message_content = "Hello!"
        mock_agents_client.messages.create.side_effect = Exception("Message creation failed")
        ...
        mock_agents_client.runs.create.assert_not_called()

    async def test_run_status_with_tool_execution_failure(self, run_manager, mock_agents_client, mock_tool_manager):
    ...
    async def test_run_manager_initialization(self, mock_agents_client, mock_tool_manager):
        """Test RunManager initialization"""
        # Act
        run_manager = RunManager(mock_agents_client, "test-assistant", mock_tool_manager)

        # Assert
        assert run_manager.agents_client == mock_agents_client
