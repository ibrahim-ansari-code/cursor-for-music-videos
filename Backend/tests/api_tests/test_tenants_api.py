...
async def created_tenant(api_client, created_landlord_property: int):
    ...
    try:
        ...
        if delete_response.status_code == 204:
            ...
            logger.warning(f"⚠️ Fixture cleanup: tenant {tenant['id']} deletion blocked by permissions (RLS issue)")
        elif delete_response.status_code == 404:
            logger.info(f"✅ Fixture cleanup: tenant {tenant['id']} already deleted")
        else:
...
class TestTenantsAPI:
    ...
    async def test_create_tenant(self, api_client, created_landlord_property: int):
        ...
        logger.info(f"✅ POST /api/tenants/ successful, created tenant ID: {tenant_id} associated with prop: {property_id}")
        ...
        try:
            ...
            # No longer expect 403 for this cleanup if property association is correct
                ...
                logger.error(f"❌ Cleanup failed for tenant {tenant_id}: DELETE returned {delete_response.status_code} - {delete_response.text[:200]}")
                pytest.fail(f"Cleanup for tenant {tenant_id} failed with status {delete_response.status_code}")
        except Exception as e:
    ...
    async def test_get_specific_tenant(self, created_tenant, api_client):
        ...
        tenant = assert_valid_json_response(response, dict, expected_status=200)
    ...
    async def test_update_tenant(self, created_tenant, api_client):
        ...
        updated_tenant = assert_valid_json_response(update_response, dict, expected_status=200)
    ...
    async def test_delete_tenant(self, api_client, created_landlord_property: int):
        ...
        tenant = assert_valid_json_response(create_response, dict, expected_status=201)
        tenant_id = tenant["id"]
