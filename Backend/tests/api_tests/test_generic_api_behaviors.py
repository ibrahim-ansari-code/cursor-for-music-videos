...
class TestGenericAPIBehaviors:
    ...
    @pytest.mark.asyncio
    async def test_health_check(self, api_client): # No auth needed, but api_client can be used
        ...
        logger.info(f"✅ API health check successful, status {response.status_code}")
    ...
    async def test_datetime_validation_in_response(self, api_client):
        ...
        if payments and len(payments) > 0:
            ...
            datetime_fields = ['created_at', 'updated_at', 'payment_date', 'due_date']
            for field in datetime_fields:
                if field in payment:
                    # Validate ISO format
                    try:
                        datetime.fromisoformat(payment[field].replace('Z', '+00:00'))
                        logger.info(f"   ✅ {field} is properly formatted: {payment[field]}")
                    except ValueError:
                        pytest.fail(f"Invalid datetime format for {field}: {payment[field]}")
    ...
    async def test_timezone_string_handling_in_query(self, api_client):
        ...
        for tz_datetime in timezone_cases:
            ...
            assert response.status_code in [200, 400], f"Timezone datetime should return 200 or 400, got {response.status_code}"
            if response.status_code == 400:
                logger.info(f"   ⚠️ {tz_datetime} rejected (might be expected)")
            else:
    ...
    async def test_method_not_allowed(self, api_client):
        ...
        assert patch_response.status_code in [405, 404], f"PATCH should return 405 or 404, got {patch_response.status_code}"
        
        logger.info(f"✅ Method not allowed test successful, status {patch_response.status_code}")
    ...
    async def test_malformed_json_payload(self, api_client):
        ...
        assert response.status_code in [400, 422], f"Malformed JSON should return 400 or 422, got {response.status_code}"
        
        logger.info(f"✅ Malformed JSON handling successful, status {response.status_code}")
