...
async def test_get_expenses_success(sample_expenses):
    ...
    with patch("Backend.api.accounting.expenses.router.service.get_expenses", new=AsyncMock(return_value=sample_expenses)):
...
async def test_get_expenses_with_filters(sample_expenses):
    ...
    with patch("Backend.api.accounting.expenses.router.service.get_expenses", new=AsyncMock(return_value=sample_expenses)) as mock_get:
        app.dependency_overrides[get_current_user] = lambda: fake_user
        app.dependency_overrides[get_session] = lambda: AsyncMock()
        ...
        with TestClientWithHost(app) as client:
            ...
            args = mock_get.await_args[0]
            assert args[2] == 123  # property_id
            assert args[3] == "Utilities"  # category
            assert args[4] == date(2024, 1, 1)  # start_date
            assert args[5] == date(2024, 1, 31)  # end_date
...
async def test_get_expenses_admin_access():
    ...
    with patch("Backend.api.accounting.expenses.router.service.get_expenses", new=AsyncMock(return_value=admin_expenses)):
...
async def test_get_expenses_unauthorized():
    ...
    with patch(
        ...
        with TestClientWithHost(app) as client:
            ...
            assert response.json()["detail"] == "Not authorized to view expenses for this property"
...
async def test_get_expense_by_id_unauthorized():
    ...
    with patch(
        ...
        with TestClientWithHost(app) as client:
            ...
            assert response.status_code == status.HTTP_403_FORBIDDEN
            assert response.json()["detail"] == "Not authorized to access this expense"
...
async def test_get_expenses_empty_result():
    ...
    with patch("Backend.api.accounting.expenses.router.service.get_expenses", new=AsyncMock(return_value=[])):
        app.dependency_overrides[get_current_user] = lambda: fake_user
        app.dependency_overrides[get_session] = lambda: AsyncMock()
...
async def test_get_expenses_date_filtering():
    ...
    with patch("Backend.api.accounting.expenses.router.service.get_expenses", new=AsyncMock(return_value=filtered_expenses)):
        app.dependency_overrides[get_current_user] = lambda: fake_user
        app.dependency_overrides[get_session] = lambda: AsyncMock()
        ...
        with TestClientWithHost(app) as client:
            ...
            data = response.json()
            assert len(data) == 1
            assert data[0]["expense_date"].startswith("2024-02-15")
