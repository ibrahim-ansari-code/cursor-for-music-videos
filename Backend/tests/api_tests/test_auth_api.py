...
class TestAuthAPI:
    ...
    async def test_auth_me_endpoint(self, api_client: APITestClient) -> None:
        ...
        if api_client.current_user:  # current_user is set in the api_client fixture
            ...
            logger.info("   Authenticated as: %s",
                        api_client.current_user.get('email'))
            ...
            logger.warning(
                "   api_client.current_user was not set by the fixture during /auth/me call by the fixture itself.")
            logger.info(
                "   Authenticated user details from current test call: id=%s, email=%s",
                data.get('id'),
                data.get('email')
            )
