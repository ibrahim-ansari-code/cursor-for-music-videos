...
def print_detailed_summary(report_data):
    ...
    if not tests:
        logger.warning("⚠️ No detailed test data in JSON report, using summary statistics")
    ...
    for test in tests:
        ...
        if '::' in nodeid:
            ...
            if file_part.startswith('test_') and file_part.endswith('_api.py'):
                api_name = file_part.replace('test_', '').replace('_api.py', '').upper()
            else:
                api_name = "OTHER"
        else:
            api_name = "UNKNOWN"
    ...
    overall_totals = {'total': 0, 'passed': 0, 'failed': 0, 'skipped': 0, 'errors': 0}
    ...
    for api_name, stats in sorted(api_groups.items()):
        ...
        print(f"  Tests: {total} | Passed: {passed} | Failed: {failed} | Skipped: {skipped} | Pass Rate: {pass_percentage:.1f}%")
...
def check_dependencies():
    ...
    try:
        ...
        if "--json-report" not in result.stdout:
            logger.warning("⚠️ pytest-json-report plugin not found. Installing...")
            try:
                subprocess.check_call([sys.executable, "-m", "pip", "install", "pytest-json-report"])
                print("✅ pytest-json-report installed")
            except subprocess.CalledProcessError:
                logger.warning("⚠️ Could not install pytest-json-report. Using basic reporting.")
                return False
    except:
