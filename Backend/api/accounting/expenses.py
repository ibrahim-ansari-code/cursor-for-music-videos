...
from Backend.models.accounting.expense import (
    Expense, ExpenseTaxDetail, ExpenseTaxDetailCreate, ExpenseCreate,
    ExpenseUpdate, ExpenseResponse, ExpenseReceiptParseDetails, ExpenseReceiptParseResponse
)
...
async def _update_expense_taxes(
    ...
    if expense_data.taxes is not None:
        tax_details_dto, calculated_total_tax_amount = _calculate_expense_taxes(
            expense_data, current_subtotal)
        new_tax_details_orm = _create_expense_tax_orm_list(
        ...
        db_expense.total_tax_amount = _recalculate_orm_taxes(
...
async def create_expense(
    ...
    try:
        tax_details_dto, total_tax_amount = _calculate_expense_taxes(expense_data, subtotal)
    ...
    tax_orm_objects = _create_expense_tax_orm_list(tax_details_dto, subtotal)
    ...
    try:
        ...
        if commit_succeeded and blob_to_delete:
            background_tasks.add_task(
                _delete_blob_with_error_handling, blob_to_delete)
...
async def delete_expense(
    ...
    try:
        ...
        if commit_succeeded and receipt_url_to_delete:
            background_tasks.add_task(
                _delete_blob_with_error_handling, receipt_url_to_delete)
